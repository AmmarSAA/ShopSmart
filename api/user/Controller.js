...
const userByEmail = async (req, res) => {
	...
	try {
		...
		//Connection to database
		await connect(process.env.MONGO_URI);
		//find one from email and fetch it's details except of password
...
const search = async (req, res) => {
	...
	try {
		...
		// Find and fetch user details excluding the password
		...
		if (results.length === 0) {
			return res.json({
				message: `Oops! No users found for the search term '${search}'.`,
			});
		}
